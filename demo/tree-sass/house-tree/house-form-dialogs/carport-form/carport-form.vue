        ...
        coverData: {
          resourcefieldBindingfnList: {
            houseName: (params) => {
              ...
              if (params.type === 'blur') {
                if(params.formData.modelData.carportShortName===undefined||params.formData.modelData.carportShortName===''){
    ...
    methods: {
      ...
      getDictionary() {
        fetchRelateData({
          ...
          if (resultData.carportTypeId && resultData.carportTypeId.length > 0) {
            //车位类型
            this.formatSelectItems("carportTypeIdItems", resultData.carportTypeId[0].dictionaryitemVos);
          }
      ...
      autoFormSubmit(formName) {
        this.autoForm.validate(formName).then(params => {
          ...
          modelData.houseType = '8';
          modelData.houseTypeEnum = "CARPORT";
