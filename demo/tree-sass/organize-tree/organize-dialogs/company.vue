<template>
	<ns-dialog :title="title" id="add_or_edit_company" :visible.sync="companyVisible" @close="dialogClose">
		<el-form :model="companyForm" class="invariable" :rules="rules" ref="companyForm" label-width="150px">
			...
			<el-form-item label="公司名称" prop="companyName">
				<el-input @blur="autoMakeMassage" size="small" v-model="companyForm.companyName" placeholder="请输入公司名称"></el-input>
			</el-form-item>
			<el-form-item label="公司编号" prop="companyCode">
				<el-input v-judgeChange = "judgeChangeParams_code" size="small" v-model="companyForm.companyCode" placeholder="请输入公司编号"></el-input>
			</el-form-item>
			<el-form-item label="公司简称" prop="companyShortName">
				<el-input v-judgeChange = "judgeChangeParams_shortName" size="small" v-model="companyForm.companyShortName" placeholder="请输入公司简称"></el-input>
			</el-form-item>
...
export default {
  directives: {
    ...
    judgeChange: {
      inserted(el, params, vnode, oldVnode) {
        el.querySelector(".el-input__inner").onblur = ()=> {// 失焦时触发
          let val = eval("vnode.context." + params.value.val);
          let preVal = eval("vnode.context." + params.value.preVal);
          if (!val || val === preVal) {// val为空或者val等于上一个值的时候，flag = false
            eval("vnode.context." + params.value.flag + " = false");
          } else if (val !== preVal) {// val不等于上一个值的时候,flag = true
  ...
  methods: {
    // 自动生成简称与编号
    autoMakeMassage() {
      if (this.companyForm.companyName) {// 有公司名称才去请求
        autoMakeMassage({ // 从后台获取简称和编号
        ...
        if (valid) {
          if (this.type === "edit") {
            ...
            companyEdit(this.companyForm).then(r => {
              ...
              // this.nodeInfo.organizationName=this.companyForm.companyName;
